...
-- EDA is an approach for data analysis that postpones the usual assumptions about what kind of model the data follow with the more direct approach of allowing the data itself to reveal its underlying structure and model. EDA employs a variety of techniques to maximize insight into a data set, uncover underlying structures, extract important variables, detect outliers and anomalies, develop parsimonious models and determine optimal factor setting.
The four types of EDA are Univariate non-graphical, univariate graphical, multivariate non-graphical and multivariate graphical. Beyond the four categories mentioned EDA further divisions based on the role (outcome or explanatory) and type (categorical or quantitative) of the variable being examined.
In my opinion EDA is the best way to summarize the main characteristics of data. It is not easy to look at a column of numbers or a whole spreadsheet and determine important characteristics of data, It may be tedious, boring and overwhelming to derive insights by looking at plain numbers.
 --
</pre>
...
<pre style="font-weight: bold; font-family: monospace; white-space: pre-wrap; background-color: #717171; border: 2px solid #981e32;">
-- The difference between r norm and r unif  is the r u norm function stimulates random variates having a specified normal distribution and r unif function generates random values of the uniform distribution.
The sample() function is used to generate a random sample of observations from the population.
 --
</pre>
...
<pre style="font-weight: bold; font-family: monospace; white-space: pre-wrap; background-color: #717171; border: 2px solid #981e32;">
-- If the 3 10-sided dice are thrown at the same time A 23 can be rolled 36 ways. The probability that a 23 can be rolled when the 3 10-sided dice are thrown at the same time on a single throw is 0.036. --
</pre>
...
sub.myrolls = subsetDataFrame(myrolls, "roll.total", "==", 12||22);

sub.myrolls;


sub.outcomes.df = subsetDataFrame(outcomes.df, "roll.total", "==", 12||22);
...
sub.myrolls = subsetDataFrame(myrolls, "roll.total", "<", 12);
...
<pre style="font-weight: bold; font-family: monospace; white-space: pre-wrap; background-color: #717171; border: 2px solid #981e32;">
-- The histogram shows that the job searches have a higher frequency and the box plot shows that the searches are not clustered at one data point. on the summary we can see that the mean is grater that the median which shows that the distribution is positively skewed. --
</pre>
...
plot(jobs.subset$job.count)

```

<pre style="font-weight: bold; font-family: monospace; white-space: pre-wrap; background-color: #717171; border: 2px solid #981e32;">
-- The histogram shows that the most searched jobs are between 0-5000. The boxplot shows how the data is scattered. --
</pre>
...
<pre style="font-weight: bold; font-family: monospace; white-space: pre-wrap; background-color: #717171; border: 2px solid #981e32;">
-- By taking a glimpse at the data or getting a general opinion the searches seem too look like taking the same trend. The parallel lines show the relationship between multiple continuous variables.The "Data analysis" key word is trending in a very high volume and the search words uses have different effects on the search results. --
</pre>
...
<pre style="font-weight: bold; font-family: monospace; white-space: pre-wrap; background-color: #717171; border: 2px solid #981e32;">
-- As we can see on the summary for week 40 the data collected under Git drops down to zero which means the data lacks continuity.The data didn't drop to zero that week rather it might not be recorded or the code crashed on that week for Git. --
</pre>
...
<pre style="font-weight: bold; font-family: monospace; white-space: pre-wrap; background-color: #717171; border: 2px solid #981e32;">
--By using box plot we will know more about our data than just a possible center value and also know the actual distribution of the data. The IQR is the distance between the third and first quartile which is considered a more stable statistic than the range of the set.
Logical inference derives from premises known or assumed to be true and stat inferential statistical test is a process of using data analysis to deduce properties of an underlying distribution of probability. Therefore, The statistical test might give us a different result than logical inference.
  --
</pre>
...
<pre style="font-weight: bold; font-family: monospace; white-space: pre-wrap; background-color: #717171; border: 2px solid #981e32;">
-- As we can see from the boxplot the Median for Java is greater than statistics, lower points for Java is roughly around 50 and for statistics it is lower which we can say that Java has greater data variety.  --
</pre>
...
<pre style="font-weight: bold; font-family: monospace; white-space: pre-wrap; background-color: #717171; border: 2px solid #981e32;">
-- For one job search it is very likely to have both keywords because most searches use recommendations and other algorithms as we type in our key words.The best approach to collecting data accurately we should be carefully analyzing the boxplot and ttest. --
</pre>
...
<pre style="font-weight: bold; font-family: monospace; white-space: pre-wrap; background-color: #717171; border: 2px solid #981e32;">
-- At the present time there is a large volume of data both structured and unstructured gets recorded, analyzed, used and stored. The term Data Provenance refers to a record trail that accounts for the origin of a piece of data(in a database, document or repository) together with an explanation of how and why it got to the present place.  Databases today do not have a good way of managing provenance data and the subject is an active research area.
Data provenance captures inputs, entity, system, and processes that influence the data of interest. This in effect provide a historical record of data and its origin and generate evidence that support forensic activities such as data dependencies and analysis, error, and compliance analysis. It emphasizes that you should strive to maintain the traceability of the data on this journey.
Data provenance is important because any of the data can be identifies and reproduced quickly without rework or reinterpretation, it is very easy explaining changing numbers, we are able to look at a data point in an output work product and quickly trace its origins back to its raw source. we can quickly know how the data was coded and it is very easy to report components based on actual work products. 
 --
</pre>
...
hometown.latitude = 47.608013; 
hometown.longitude =  -122.335167;
my.radius = 13; my.units = "mi"; #miles


 hometown.info = getNeighborsFromLatLong(13, 47.608013, -122.335167, "mi");
...
<pre style="font-weight: bold; font-family: monospace; white-space: pre-wrap; background-color: #717171; border: 2px solid #981e32;">
-- Considering the data visualized I liked the Voroni diagram better because it lists the name of the capital and easy to understand.My least favorite is the "base" plot because There are some Astrikes outside the map that has no meaning. --
</pre>
[
...
<pre style="font-weight: bold; font-family: monospace; white-space: pre-wrap; background-color: #717171; border: 2px solid #981e32;">
-- 
The Haversine distance uses the angular distance between two points on the surface of a sphere but the second one which uses the shortest distance. The most accurate one is the Meeus
 --
</pre>
[
...
<pre style="font-weight: bold; font-family: monospace; white-space: pre-wrap; background-color: #717171; border: 2px solid #981e32;">
-- The longtiude/latitude changed into Miles. --
</pre>
[
...
<pre style="font-weight: bold; font-family: monospace; white-space: pre-wrap; background-color: #717171; border: 2px solid #981e32;">
-- Based on the 'hclust.complete.dist.educlidean' The U.s regions are subdivided into 12 groups.The function performed a hierarchical cluster analysis using a set of similarities for the 50 states being clustered. The North and South Dekota are clustered together on Sub tree 12. Indiana and Kentuky are clustured togehter on sub tree 6.North and South Carolina are clustered together on Sub tree 7. --
</pre>
...
movies.50$cluster.arbitrary[is.na(movies.50$millionsAdj2000)] <- 'cluster 1';
str(movies.50);
movies.50$cluster.arbitrary[movies.50$millionsAdj2000 < 1] <- 'cluster2'
movies.50$cluster.arbitrary[movies.50$millionsAdj2000 >= 1 & movies.50millionsAdj2000 < 5] <- 'cluster3'
movies.50$cluster.arbitrary[movies.50$millionsAdj2000 >= 5 & movies.50millionsAdj2000 < 50] <- 'cluster4'
...
<pre style="font-weight: bold; font-family: monospace; white-space: pre-wrap; background-color: #717171; border: 2px solid #981e32;">
-- This graph is very unique and easy to understand the temperature of Helena, Montana and Baton Rouge, Louisiana.The one thing I liked about this graph is the Y-axis temperature and X-axis Months are clearly displayed and it used a sun emoji to show the highest temperature recorded. --
</pre>
[
...
<pre style="font-weight: bold; font-family: monospace; white-space: pre-wrap; background-color: #717171; border: 2px solid #981e32;">
-- Eventhough the gridlines are abit small and hard to see the numbers on the Y-axis this approach is very good to compare the two cities temperature.I beleive the graph uses the blue dots as rain and dark blue dots as snow. --
</pre>
[
What do you like about this graphic?  
...
<pre style="font-weight: bold; font-family: monospace; white-space: pre-wrap; background-color: #717171; border: 2px solid #981e32;">
-- The states that are in the same cluster are
Cluster 1 – AL, AK, AZ, AR, CA
Cluster 2 – CO, CT, DE, FL, GA, HI
Cluster 3 – ID
Cluster 4 – IL
Cluster 5 – IN, IA, KS, KY, LA
Cluster 6 – ME, MD, MA, MI
Cluster 7 – MN, MS, MO
Cluster 8 – MT, NE, NV, NH, NJ, NM, NY
Cluster 9 – NC, ND, OH
Cluster 10 - OK, OR, PA, RI, SC, SD
Cluster 11 – TN
Cluster 12 – TX, UT, VT, VA, WA, WV, WI, WY
Each cluster has the months highest temperature
 --
</pre>
...
<pre style="font-weight: bold; font-family: monospace; white-space: pre-wrap; background-color: #717171; border: 2px solid #981e32;">
-- We can see on this example even though the months are changing we are keeping record of the highest and lowest temperature of that month. --
</pre>
...
<pre style="font-weight: bold; font-family: monospace; white-space: pre-wrap; background-color: #717171; border: 2px solid #981e32;">
-- The correlation of the Lowest temperature on eahc month varies as the numeric factors.--
</pre>
...
x = c(20,20,10,10,20,20);  ## change these values and discuss ...
x.labels = c("collection", "organization", "EDA analysis", "CDA analysis", "interpretation", "presentation");
x.colors = c("blue", "lightgreen", "green", "darkgreen", "orange", "red");
...
<pre style="font-weight: bold; font-family: monospace; white-space: pre-wrap; background-color: #717171; border: 2px solid #981e32;">
--1. In my opinion the values given to each sections of statistics are distributed fairly.
Collecting the data and organizing it are the very first steps that we have to take and 20 out of 100 for each is very fair.
EDA and CDA are very important to analyze the data sets and summarize the characteristics and also 'discourse’ is to signal patterns of language use and meanings that belong to specific social or disciplinary domains is also important and 10 out of 100 for each is again fair.
After we gather our data and summarize it in different formats the next step is interprating it and guiding the data to the best possible outcome disired. After the data is interpreted we want to present our neat data based on the requirements. For example we can't show a presentation of codes to the CEO of a company with little or no knowledge of coding. There fore Interpretation and presentation can take the last 40 out of the 100.

2.I haven't finished all the core courses but the core courses I took have helped me develop a great understanding of Data Analytic.
The computer science classes introduced me to writing small codes to solve problems
The statistics classes helped me to understand data collection and interpretation of Data
The Data Analysis classes helped me understand different ways of Data visualization and Data Manipulation.

4.This class have helped me so much in both coding and critical thinking. It also helped me to get an idea of what I should expect in my career. For that Thank you very much Professor!
--
</pre>
...
# 
 points(0.65, 0.78, pch=20, col="#981e32");
 text(0.70, 0.89, "Gabriel Tessema", col="#981e32", cex=0.75, srt = 45, pos=3);
# 
# ## evaluate the Course Categories of Tools of the Trade
# ## give them a score
# 
 points(0.85, 0.90, pch=20, col="#981e32");
 text(0.90, 0.98, "Math(s)", col="#981e32", cex=0.5, srt = 45, pos=3);
# 
# 
 points(0.65, 0.70, pch=20, col="#981e32");
 text(0.70, 0.75, "Computer Science", col="#981e32", cex=0.5, srt = 45, pos=3);
# 
 points(0.46, 0.50, pch=20, col="#981e32");
 text(0.50, 0.55, "Machine learning", col="#981e32", cex=0.5, srt = 45, pos=3);
# 
 points(0.78, 0.89, pch=20, col="#981e32");
text(0.80, 0.93, "Statistics", col="#981e32", cex=0.5, srt = 45, pos=3);
# 
 points(0.80, 0.85, pch=20, col="#981e32");
 text(0.86, 0.91, "Data analytics", col="#981e32", cex=0.5, srt = 45, pos=3);
# 
#You have a track (e.g., Business)
 points(0.55, 0.67, pch=20, col="#981e32");
text(0.63, 0.73, "Business", col="#981e32", cex=0.5, srt = 45, pos=3);
